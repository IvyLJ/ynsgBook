...
functions of all different shapes together in programs =>
(1) face incompatibilities(不兼容性) in the number/order/type of inputs~~~~
(2)  need to specify some inputs at different times

for stylistic purposes of readability =>  define functions in a way that hides their inputs entirely!

## 1. All for One

a simple helper that wraps a function call to ensure only one argument will pass through. enforcing function is treated as unary:
...
the shorter `=>` arrow function syntaxas:
...
**Note:** No question this is more terse(简洁), sparse(稀疏) even. But it loses more in overall readability with the functions all being anonymous, and by obscuring the scope boundaries(模糊了范围边界), making dec~~~~iphering(破译) closure a little more cryptic(晦涩).

A commonly cited example for using `unary(..)` is `map(..)` utility (see [Chapter 9, "Map"](ch9.md/#map)) and `parseInt(..)`:
` map(..)` calls a mapper function for each item in a list, and each time it invokes the mapper function, it passes in three arguments: `value`, `idx`, `arr`.

trying to use something as a mapper function that will behave incorrectly if it's passed too many arguments. Consider:
...
var identity =
    v =>
        v;
用途：imagine you'd like to split up a string using a regular expression, but the resulting array may have some empty values in it. To discard those, we can use JS's `filter(..)` array operation (see [Chapter 9, "Filter"](ch9.md/#filter)) with `identity(..)` as the predicate:
...
Because `identity(..)` simply returns the value passed to it, JS coerces（强制转换） each value into either `true` or `false`, and that determines whether to keep or exclude each value in the final array.
...
## 2. Adapting Arguments to Parameters
...
A function takes multiple arguments：specify some of those up front and leave the rest to be specified later：
...
Q: set up several API calls where the URLs are known up front, but the data and the callback to handle the response won't be known until later.

A: make a new function that still calls `ajax(..)` ,  sets the first argument to the API URL , waiting to accept the other two arguments later:
...
Manually(手动) specifying these function call wrappers(函数调用包装器) is certainly possible, but it may get quite tedious（乏味）, especially if there will also be variations with different arguments preset（预设）, like:
...
One way to articulate（阐明） what's going on is that the `getOrder(data,cb)` function is a *partial application* of the `ajax(url,data,cb)` function. This terminology comes from the notion that arguments are *applied* to parameters at the function call-site. And as you can see, we're only applying some of the arguments up front -- specifically, the argument for the `url` parameter -- while leaving the rest to be applied later.

To be a tiny bit more formal about this pattern, partial application is strictly a reduction in a function's arity（复杂度）; remember, that's the number of expected parameter inputs. We reduced the original `ajax(..)` function's arity from 3 to 2 for the `getOrder(..)` function.
...
**Note:** Version 2 has an extra layer of function wrapping involved. We'll be wrapping many layers of functions onto each other as we progress through the text. Remember, this is *function*al programming!
...
（是把接受多个[参数](https://zh.wikipedia.org/wiki/%E5%8F%83%E6%95%B8_(%E7%A8%8B%E5%BC%8F%E8%A8%AD%E8%A8%88)) 的[函数](https://zh.wikipedia.org/wiki/%E5%87%BD%E6%95%B0 "函数")变换成接受一个单一参数（最初函数的第一个参数）的函数，并且返回接受余下的参数而且返回结果的新函数的技术）
...
### 4.1 Visualizing Curried Functions~~~~
...
### 4.2 Why Currying and Partial Application?
...
### 4.3 Currying More Than One Argument?
