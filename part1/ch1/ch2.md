...
Functional Programming is **not just programming with the `function` keyword.**  functions really *are* at the center of FP. And it's how we use functions that makes our code *functional*.

~~~~But how sure are you that you know what *function* really means?

## What Is a Function?~~~~

"What is a function?" : a function is a collection of code that can be executed one or more times.~~~~
...
### Brief Math Review<p align="center"><img src="images/fig1.png" width="40%">
...
functions must expect input

"arguments"& "parameters". So what's that all about?
...
As of ES5 (and strict mode, specifically), `arguments` is considered by some to be sort of deprecated; it's now commonly suggested that you avoid using it whenever possible.

However, I suggest that `arguments.length`, and only that, is OK to keep using for those cases where you need to care about the passed number of arguments.

Be careful: **never** access arguments positionally, like `arguments[1]`. Stick to `arguments.length` only, and only if you must.

A function signature that accepts an indeterminate amount of arguments is referred to as a variadic function. FPer wants to avoid these where possible.
...
What exactly do we mean by anonymous（匿名）, by the way? Specifically, functions have a `name` property that holds the string value of the name the function was given syntactically（句法上）, such as `"helloMyNameIs"` or `"namedFunctionExpr"`. This `name` property is most notably（明显地） used by the console/developer tools of your JS environment to list the function when it participates（参加） in a stack trace（堆栈跟踪） (usually from an exception).

Anonymous functions are generally displayed as `(anonymous function)`.

If you've ever had to debug a JS program from nothing but a stack trace of an exception（堆栈跟踪异常）, you probably have felt the pain of seeing `(anonymous function)` appear line after line. This listing doesn't give a developer any clue whatsoever as to the path the exception came from. It's not doing the developer any favors.
...
As of ES6, anonymous function expressions are in certain cases aided by *name inferencing*（名称推断）. Consider:
...
But beware, not all syntactic forms benefit from name inferencing. Probably the most common place a function expression（函数表达式） shows up is as an argument to a function call（作为函数调用的参数）:
...
There are other benefits to a function being named besides the debugging question. First, the syntactic name (aka lexical name词法名称) is useful for internal self-reference（内部自引用）. Self-reference is necessary for recursion（递归） (both sync and async) and also helpful with event handlers（事件处理程序）.
...
You virtually never see IIFEs using names for their function expressions, but they should. Why? For all the same reasons we just went over: stack trace debugging(堆栈跟踪调试), reliable self-reference(可靠的自引用), and readability(可读性). If you can't come up with any other name for your IIFE, at least use the word IIFE:
...
**named functions are always more preferable to anonymous functions.** As a matter of fact, there's basically never a case where an anonymous function is more preferable. don't really have any advantage .

**Name every single function.**
if you unable to come up with a good name for some function =>
don't fully understand that function's purpose yet -- or it's just too broad or abstract. =>
re-design the function until this is more clear =>
a name will become more apparent.

to name something well => understand it better, later, =>  even refactor its design for improved readability and maintainability.

### Functions Without `function`

full canonical syntax for functions vs `=>` arrow function syntax.
...
But there's another thing we omitted(省略). Did you spot it? The `getPreferredName` function name.

That's right; `=>` arrow functions are lexically anonymous; there's no way to syntactically provide it a name.
...
The case for excitement over `=>` in the FP world is primarily that it follows almost exactly from the mathematical notation for functions(函数的数学符号), especially in FP languages like Haskell. The shape of `=>` arrow function syntax communicates mathematically.

in favor of `=>` is that by using much lighter-weight syntax, we reduce the visual boundaries between functions which lets us use simple function expressions much like we'd use lazy expressions -- another favorite of the FPer.
...
**Note:** We're tackling a topic that we'll ultimately conclude we shouldn't use. Why!? Because the topic of `this` has implications(影响) for other topics covered later in this book.
(1) our notions of function purity are impacted by `this` being essentially an implicit（隐式） input to a function (see [Chapter 5](ch5.md)).
(2) our perspective on `this` affects whether we choose array methods (`arr.map(..)`) versus standalone utilities (`map(..,arr)`) (see [Chapter 9](ch9.md)).
(3) Understanding `this` is essential to understanding why `this` really should *not* be part of your FP!

JavaScript `function`s have a `this` keyword that's automatically bound per function call. it provides an object context for the function to run against.

`this` is an implicit parameter input for your function（this是函数的隐式参数input）.
...
Of course, if `this` can be input into a function implicitly, the same object context could be sent in as an explicit（显式的） argument:
